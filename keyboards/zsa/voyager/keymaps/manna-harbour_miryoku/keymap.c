...
const uint16_t PROGMEM bspc_and[]      = {LT(U_NUM, KC_BSPC), LCTL_T(KC_A), COMBO_END};
...
combo_t key_combos[] = {COMBO(l_y_p, SELWORD), COMBO(q_j, QK_LEAD), COMBO(q_j_v, AC_TOGG), COMBO(j_v_d, ALT_TAB), COMBO(j_v, KC_ESC), COMBO(x_h, KC_GRV), COMBO(h_slsh, KC_MINS), COMBO(h_comm, KC_LT), COMBO(h_dot, KC_GT), COMBO(slsh_comm, KC_EQL), COMBO(comm_dot, KC_SCLN),
                        // COMBO(bspc_c, KC_ESC),
                        COMBO(bspc_ac, KC_SCLN), COMBO(slsh_comm_dot, DOT_SLASH_UPDIR),

                        COMBO(bspc_ip, BSPCIP), COMBO(bspc_iw, BSPCIW), COMBO(bspc_ep, BSPCEP), COMBO(bspc_ew, BSPCEW),

                        COMBO(bspc_every, BSPCEV_EVERY), COMBO(bspc_you, BSPCU_YOU), COMBO(bspc_and, BSPCA_AND), COMBO(bspcw_with, BSPCW_WITH), COMBO(bspcf_for, BSPCF_FOR), COMBO(bspch_here, BSPCH_HERE), COMBO(bspct_the, BSPCT_THE), COMBO(bspcm_ment, BSPCM_MENT), COMBO(bspcg_ing, BSPCG_ING), COMBO(bspco_ough, BSPCO_OUGH), COMBO(bspcol_ould, BSPCOL_OULD), COMBO(bspci_ion, BSPCI_ION), COMBO(bspcis_ions, BSPCIS_IONS), COMBO(bspcta_that, BSPCTA_THAT), COMBO(bspcq_que, BSPCQ_QUE), COMBO(bspci_key, BSPCK_KEY), COMBO(bspcts_this, BSPCTS_THIS), COMBO(bspcdn_dont, BSPCDN_DONT), COMBO(bspcit_in_the, BSPCIT_IN_THE), COMBO(hv_have, HV_HAVE), COMBO(qk_qmk, QK_QMK), COMBO(kb_keyboard, KB_KEYBOARD), COMBO(wa_what, WA_WHAT),

                        COMBO(thumbcombos_base_right, LT(U_FUN, KC_DEL)), COMBO(thumbcombos_base_left, LT(U_MEDIA, KC_ESC)), COMBO(thumbcombos_nav, KC_DEL), COMBO(thumbcombos_mouse, KC_BTN3), COMBO(thumbcombos_media, KC_MUTE), COMBO(thumbcombos_num, KC_DOT),
#    if defined(MIRYOKU_LAYERS_FLIP)
